...
public class CommentRepository {
    ...
    public Optional<CommentDto> postComment(CommentDto comment) {
        ...
        List<CommentDto> result = jdbcTemplate.query("SELECT comment.*, user.profile_pic FROM instapic.comment AS comment INNER JOIN instapic.user AS user ON comment.user_id = user.user_id WHERE comment.article_id = ? AND user_id = ? ORDER BY comment_id DESC;", commentDtoRowMapper(), comment.getArticleId(), comment.getUserId());
